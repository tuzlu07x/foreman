...
export function buildStatusBarLayout(layout: Layout): StatusBarLayout {
  if (layout === "wide") {
    return {
      rows: [
        "[h] help · [a] agents · [v] providers · [V] services · [c] chat · [g] settings · [k] keys · [l] logs · [p] policy · [s] sessions · [q] quit",
      ],
      showVersion: true,
    };
  }
  if (layout === "medium") {
    return {
      rows: ["[h] [a] [v] [V] [c] [g] [k] [l] [p] [s] [q]"],
      showVersion: true,
    };
  }
  return {
    rows: ["nav:    [h] [a] [v] [V] [c] [g] [k] [l] [p] [s]", "system: [q]"],
    showVersion: false,
  };
}
